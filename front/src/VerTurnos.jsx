...
export const VerTurnos = () => {
    ...
    const eliminarTurno = async (turnoId) => {
        const confirmar = window.confirm("¿Estás seguro de que querés eliminar este turno?");
        if (!confirmar) return;
                            ...
                            {fechaVisible[fecha] && (
                                <ul className="space-y-4 mt-4">
                                    {turnosPorFecha.map((turno) => (
                                        <li
                                            key={turno.id}
                                            className={`relative p-6 border rounded-2xl flex flex-col justify-between items-center gap-4 shadow-lg border-l-8 ${turno.estado === 'disponible' ? 'bg-green-50  border-l-green-500' : 'bg-red-50 border-l-red-500'
                                                }`}
                                        >
                                            <div className="flex flex-col justify-center items-center gap-4">
                                                ...
                                                {turno.nombre && (
                                                    <span className="text-sm text-center text-gray-600 uppercase">
                                                        Reservado por: <br /> {turno.nombre} <br />{turno.dni} <br />{turno.telefono}
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex items-center gap-4 flex-wrap justify-center">
                                                <span
                                                    className={`text-sm font-bold uppercase ${turno.estado === 'reservado' ? 'text-red-500' : 'text-green-600'
                                                        }`}
                                                >
